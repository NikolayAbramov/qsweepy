...
class Device():
	def __init__(self, serial, fpga_config = True, adc_config = True):
		...
		self.fpga_firmware = module_dir + "\\_ADS54J40\\qubit_daq.rbf"
	...
	def fpga_config(self, source = None, firmware = None):
		...
		self.usb_reset()
		self.dev.ctrl_transfer(RQ_TYPE_DEV_VEND_WR, VEND_RQ_FPGA_CONF_INIT, 0, 0, to_bytes(len(firmware)+2,4) )
		self.dev.write( EP_OUT, firmware+bytes([0,0]))
	...
	def read_reg(self,base, offset):
		address = base + offset
		Value, Index = mk_val_ind(address)
		data = frombuffer(self.dev.ctrl_transfer(RQ_TYPE_DEV_VEND_RD, VEND_RQ_REG_READ, Value,Index,4), dtype = uint32odd)[0]
	...
	def get_data(self):
		...
		data = frombuffer(self.dev.read(EP_IN, data_len*4), dtype = dtype(int16))
